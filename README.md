...
This repository contains the reviewed implementation handoff, public governance artifacts for Stage 0 risk proofs, the bounded offline product foundation authorized by [ADR 0002](docs/adr/0002-local-first-product-foundation.md), and the offline manual-entry prototype authorized by [ADR 0003](docs/adr/0003-offline-manual-entry-prototype.md).
...
Do not build the complete app or create service, provider, or production external state before these risks are understood. Offline S1-A contract and URL-policy core work may proceed only within [ADR 0002](docs/adr/0002-local-first-product-foundation.md). [ADR 0003](docs/adr/0003-offline-manual-entry-prototype.md) separately permits one transient manual-entry surface that invokes the local syntax validator and displays content-free status; it permits no connected check, persistence, telemetry, destination action, or external distribution. Owner-local Apple automatic signing remains the sole external-state exception: it may create or fetch development provisioning state, but is neither proof evidence nor authority for entitlements, TestFlight, or release. The exact build sequence and stop gates are in the [implementation plan](docs/11-implementation-plan.md).

## Local verification

Open `HezoLink.xcodeproj` in Xcode for the minimal iOS app shell, bounded manual-entry prototype, shared core sources, and iOS test target. The same core sources remain a root Swift package for command-line and cross-platform testing. The prototype exercises local URL syntax validation and displays only finite content-free status. It does not perform a threat check, navigate to the submission, use the network, or create persistent state. The shared `HezoLink` scheme runs all core tests with Product > Test.

For a signed device build, either select the owner-local Team and bundle identifier in Xcode or copy `Config/Signing.local.example.xcconfig` to `Config/Signing.local.xcconfig` and replace the two placeholder values. Xcode's Signing & Capabilities editor may write the selected identifiers into the tracked project file; keep that local project change uncommitted. The ignored local xcconfig avoids that dirty-project state. Keep certificates, private keys, and provisioning profiles in Xcode, Keychain, or Apple-managed stores—not in either xcconfig. Simulator builds require no signing identity.

The current Swift foundation and prototype have no third-party executable-code dependencies; the core consumes reviewed vendored public data and performs no network access:

- `packages/contracts/` contains the strict Check Request V1 and bounded Pending Check Response V1 OpenAPI/JSON Schema components and offline schema fixtures.
...
- `packages/public-suffix-list/` records the pinned, licensed Public Suffix List source, integrity manifest, official test corpus, and reviewed manual lifecycle policy.
- `Sources/HezoLinkCore/` contains bounded contract values, the local URL-input syntax preflight, the offline registrable-domain classifier, and the constant log redactor used by the prototype and later manual-input surfaces.

The URL-input preflight preserves the exact sensitive submission for transient analysis and rejects known ambiguous or prohibited syntax. Syntax profile 2 pins the [IANA Special-Use Domain Names registry](https://www.iana.org/assignments/special-use-domain-names/special-use-domain-names.xhtml) revision dated 2026-05-22 and adds oracle-backed IDNA-hyphen, numeric-ending host, and RFC 5952 IPv6 rules. Its offline tests replay pinned Unicode 17 IDNA data, a fixed Web Platform Tests URL-parser corpus, and project-authored RFC IP-syntax cases; acceptance remains deliberately narrower than a general-purpose browser parser. It is not a provider canonicalizer, destination-reachability or safety decision, durable URL representation, navigation capability, or consumer screen. This batch also pins the full ICANN and PRIVATE Public Suffix List and adds offline public-suffix and registrable-domain classification. The static snapshot is classification data only; it provides no domain-validity, DNS-existence, reachability, safety, ownership, affiliation, verdict, or enforcement evidence. A complete special-address registry/classifier, ingress-to-egress parser equivalence, and the release branch-coverage gate remain required before that broader boundary can be called complete.
...
Application code: the bounded local S1-A foundation is authorized by [ADR 0002](docs/adr/0002-local-first-product-foundation.md), and the transient offline manual-entry prototype is authorized by [ADR 0003](docs/adr/0003-offline-manual-entry-prototype.md). The code contains the offline contract core, strict Check Request V1 and bounded Pending Check Response V1 data contracts, URL-input policy primitives with executable tests, and a local syntax-status screen. It contains no connected consumer check, network client, persistence, provider integration, crawler, URL Filter, App Attest, analytics, or measurement behavior. Owner-local automatic signing may create Apple development provisioning state as the sole non-proof exception. Nothing here makes a Stage 0, Stage 1, Stage 2, release, or production-readiness claim.
