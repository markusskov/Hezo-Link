...
- `packages/iana-address-registries/` records four pinned IANA registry snapshots, rights evidence, a deterministic projection, and strict provenance and integrity manifests.
- `Sources/HezoLinkCore/` contains bounded contract values, the local URL-input syntax preflight, the offline registrable-domain and address-registry classifiers, and the constant log redactor used by the prototype and later manual-input surfaces.

The URL-input preflight preserves the exact sensitive submission for transient analysis and rejects known ambiguous or prohibited syntax. Syntax profile 2 pins the [IANA Special-Use Domain Names registry](https://www.iana.org/assignments/special-use-domain-names/special-use-domain-names.xhtml) revision dated 2026-05-22 and adds oracle-backed IDNA-hyphen, numeric-ending host, and RFC 5952 IPv6 rules. Its offline tests replay pinned Unicode 17 IDNA data, a fixed Web Platform Tests URL-parser corpus, and project-authored RFC IP-syntax cases; acceptance remains deliberately narrower than a general-purpose browser parser. It is not a provider canonicalizer, destination-reachability or safety decision, durable URL representation, navigation capability, or consumer screen. The core also pins the full ICANN and PRIVATE Public Suffix List and a four-source IANA address profile for deterministic offline classification. These static snapshots provide no domain or address validity, DNS existence, reachability, safety, ownership, affiliation, verdict, connection, or enforcement evidence. Ingress-to-egress parser equivalence and the release branch-coverage gate remain required before that broader boundary can be called complete.
