...
The Swift core exposes `VerdictV1` as the canonical structural verdict type and retains `Verdict` as a source-compatible alias. Both names contain exactly the five fields `label`, `recommended_action`, `confidence`, `evaluated_scope`, and `reasons`, and enforce the same exhaustive 16-pair matrix: the five admitted pairs above and the other eleven rejected pairs. The alias adds no new vocabulary, wire value, authority, or behavior.

The type alone does not authorize a completed response; establish the completeness or freshness required for `no_known_danger`; choose `warn` versus `retry` for `unknown`; select or support evidence, rank or deduplicate reasons; decide automatic block eligibility; or authorize an endpoint, network I/O, persistence, UI, or runtime behavior.

`confidence` is an open, forward-compatible bounded category, not an internal score or consumer probability. The Swift core exposes `ConfidenceCategoryV1` as the canonical type and retains `ConfidenceCategory` as a source-compatible alias; both names preserve the same grammar, while `low`, `medium`, and `high` are published V1 conveniences rather than an exhaustive enum. The category does not determine policy or completeness. The Swift core exposes `EvaluatedScopeV1` as the canonical evaluated-scope type and retains `EvaluatedScope` as a source-compatible alias; both names preserve the same forward-compatible grammar, and `exact_url` remains the only published V1 scope meaning. Reasons use stable codes and localization keys, the grammar above, and a maximum of five items. `VerdictReasonV1` is the canonical Swift reason type, and `VerdictReason` remains a source-compatible alias; the exact six-field contract and wire representation are unchanged. `VerdictReasonsV1` is the canonical Swift ordered collection and `VerdictReasons` remains a source-compatible alias; both preserve the exact zero-through-five-item input ordering and duplicate-permitting structural semantics without selecting or supporting evidence, ranking or deduplicating reasons, deciding currentness, reading a clock, authorizing completed-response semantics, or introducing new vocabulary or wire values. Swift deliberately retains `observedAt: Date`, while `HezoJSON` applies `CanonicalInstantV1` wire semantics to `observed_at`. The type and timestamp do not define evidence support or ranking, decide evidence freshness, authorize clock use, or approve completed-response semantics. Server fallback copy is bounded and derived only from approved reason data.
