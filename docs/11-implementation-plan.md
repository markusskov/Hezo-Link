...
[ADR 0002](adr/0002-local-first-product-foundation.md) now authorizes only the bounded, local S1-A foundation to overlap the incomplete Stage 0 proofs. This overlap creates no external state and does not satisfy the Stage 0 exit. Stage 2, live providers, cloud deployment, Apple submission/distribution, crawler execution, and any user-facing release remain blocked until their applicable gates pass.
...
The stage rule applies to stages, not to independent proofs inside Stage 0. Start with S0-A. After its relevant decisions are recorded, S0-B through S0-F may proceed in parallel when they do not share files or infrastructure. Under [ADR 0002](adr/0002-local-first-product-foundation.md), the bounded local S1-A foundation may proceed in parallel without creating external state; Stage 2 and every live or user-facing path still require the complete Stage 0 exit.
...
| S1-A | First product-foundation PR | Backend/iOS leads | Authorized locally by ADR 0002: contract/error envelope, minimal check schemas, URL-policy parsing/IDNA/redaction, official parser/IP fixtures, property tests, and local verification. No production API, database, UI, crawler, vendor connector, cloud deployment, or gate claim. |
