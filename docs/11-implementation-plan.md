...
- O-020 has an owner-approved outcome for Stage 0 proof authorization and exit; [ADR 0002](adr/0002-local-first-product-foundation.md) authorizes bounded local S1-A work only and does not resolve this proof-timing decision;
- no critical open decision in document 12 remains ownerless.

If URL Filter approval appears infeasible, decide explicitly whether the manual-only product remains commercially worthwhile before continuing. O-014 does not waive a failed URL Filter exit criterion by itself: continuing beyond the bounded local S1-A exception as a manual-only product requires an Accepted rescope ADR that updates the product promise, Stage 0 exit criteria, build order, tests, and affected documents before Stage 2 or any user-facing release.

[ADR 0002](adr/0002-local-first-product-foundation.md) now authorizes only the bounded, local S1-A foundation to overlap the incomplete Stage 0 proofs. This overlap creates no product, service, provider, or production state and does not satisfy the Stage 0 exit. Owner-local Apple automatic signing for the inert shell is the sole external-state exception; any development provisioning state it creates or fetches is not proof evidence. Stage 2, live providers, cloud deployment, Apple submission/distribution, crawler execution, and any user-facing release remain blocked until their applicable gates pass.
...
The stage rule applies to stages, not to independent proofs inside Stage 0. Start with S0-A. After its relevant decisions are recorded, S0-B through S0-F may proceed in parallel when they do not share files or infrastructure. Under [ADR 0002](adr/0002-local-first-product-foundation.md), the bounded local S1-A foundation may proceed in parallel without creating product, service, provider, or production state. Owner-local automatic signing may create Apple development provisioning state only for the inert shell and cannot support a proof claim. Stage 2 and every live or user-facing path still require the complete Stage 0 exit.
