...
- O-020 has an owner-approved outcome for Stage 0 proof authorization and exit; [ADR 0002](adr/0002-local-first-product-foundation.md) authorizes bounded local S1-A work and [ADR 0003](adr/0003-offline-manual-entry-prototype.md) authorizes the offline S1-B manual-entry prototype, but neither resolves this proof-timing decision;
- no critical open decision in document 12 remains ownerless.

If URL Filter approval appears infeasible, decide explicitly whether the manual-only product remains commercially worthwhile before continuing. O-014 does not waive a failed URL Filter exit criterion by itself: continuing beyond the bounded local S1-A and S1-B exceptions as a manual-only product requires an Accepted rescope ADR that updates the product promise, Stage 0 exit criteria, build order, tests, and affected documents before Stage 2 or any user-facing release.

[ADR 0002](adr/0002-local-first-product-foundation.md) authorizes the bounded local S1-A foundation to overlap the incomplete Stage 0 proofs. [ADR 0003](adr/0003-offline-manual-entry-prototype.md) separately authorizes S1-B: one transient offline manual-entry surface using the existing local validator and content-free status. Neither exception satisfies the Stage 0 exit or permits a connected check, persistence, telemetry, destination action, service/provider/production state, or external distribution. Owner-local Apple automatic signing remains the sole external-state exception; any development provisioning state it creates or fetches is not proof evidence. Stage 2, live providers, cloud deployment, Apple submission/distribution, crawler execution, and any user-facing release remain blocked until their applicable gates pass.

## Stage 1 — contracts, policy core, and storage

### Goal

Create deterministic foundations without an externally connected, persistent, or distributable user product. The bounded S1-B offline interaction prototype is the sole UI exception before the complete Stage 0 exit.
...
The stage rule applies to stages, not to independent proofs inside Stage 0. Start with S0-A. After its relevant decisions are recorded, S0-B through S0-F may proceed in parallel when they do not share files or infrastructure. Under [ADR 0002](adr/0002-local-first-product-foundation.md), the bounded local S1-A foundation may proceed in parallel. Under [ADR 0003](adr/0003-offline-manual-entry-prototype.md), S1-B may add only the transient offline interaction prototype after the local validator exists. Owner-local automatic signing remains the sole external-state exception and cannot support a proof claim. Stage 2, every connected path, and every externally distributed user-facing path still require the complete Stage 0 exit.
...
| S1-A | First product-foundation PR | Backend/iOS leads | Authorized locally by ADR 0002: contract/error envelope, minimal check schemas, an inert Xcode app shell for build/simulator/owner-local signing diagnostics, URL-policy parsing/IDNA/redaction, official parser/IP fixtures, property tests, and local verification. No consumer feature flow, production API, database, crawler, vendor connector, cloud deployment, or gate claim. |
| S1-B | Offline manual-entry prototype | iOS lead | Authorized locally by ADR 0003 after the validator exists: one manual field, explicit in-process validation action, finite content-free syntax status, clear behavior, and deterministic offline tests. No network or provider call, persistence or history, analytics or measurement, automatic clipboard read, share/QR intake, submitted-URL navigation, verdict, URL Filter, App Attest, external distribution, or gate claim. |
